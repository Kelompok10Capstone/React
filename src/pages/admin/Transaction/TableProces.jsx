...
const TableProces = () => {
  ...
  const getProses = async () => {
    try {
      const responseProses = await api.get(
        `admin/transactions/product/?product=${productP}&status=${statusP}&page=${pageProses}&limit=${limitProses}`
      );
  ...
  const getTransactionByStatusQuery = async () => {
    try {
      const responseProses = await api.get(
        `admin/transactions/status/search/?status=${statusP}&query=${query}&page=${pageProses}&limit=${limitProses}`
      );
      ...
      <div className="row justify-content-end mb-5">
        <form className="search-transaction">
          <Search
            ...
            type="text"
            onChange={(e) =>
              setQuery(e.target.value.toLowerCase()) || setPageProses(1)
            }
          />
        </form>
      </div>
